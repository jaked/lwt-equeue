...
and 'a communication =
  { performed: int ref;  (* -1 if not performed yet, set to the number *)
    ...
    condition: unit Lwt.u;              (* To restart the blocked thread. *)
...
let basic_sync abort_env genev =
  ...
  let condition_t, condition_u = Lwt.wait() in
  let bev = Array.create (Array.length genev)
                         (fst (genev.(0)) performed condition_u 0) in
  for i = 1 to Array.length genev - 1 do
    bev.(i) <- (fst genev.(i)) performed condition_u i
  done;
  ...
  if poll_events 0 then Lwt.wakeup condition_u ()
  else begin
